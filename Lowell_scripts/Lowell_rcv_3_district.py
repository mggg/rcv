...
(STV) process for each district in a 3-district plan.
...
def scenario_generator(num_seats, votes, vote_vec, perm_white_vec, perm_coal_vec, pct_crossover_vec):
    ...
    for i in range(len(vote_vec)):
        # polarized votes
        for vote in range(math.ceil(votes*vote_vec[i]*(1.0-pct_crossover_vec[i]))): 
...
#white, hispanic, asian, other each district in sample plan
white = [0.5846965769228165, 0.5840242048290024, 0.589693289191318]
hisp = [0.16402528914033881, 0.12772042221471666, 0.22701698157746703]
asian = [0.18264876017929343, 0.23107219892958672, 0.10868367177903482]
other = [0.06862937375755118, 0.05718317402669415, 0.07460605745218007]

rcv_output = pd.DataFrame(columns = ["Turnout", "Crossover", "CVAP", "num_seats", \
...
cvap_list = [[white[i], hisp[i], asian[i], other[i]] for i in range(len(white))]
for cvap in cvap_list:
    cvap_vec = cvap
...
###################### Edit Inputs Below ######################
            ...
            num_votes = 5000 
            num_runs = 200 
            num_seats = 3 #elected in each of the 3 districts
...
###################### Edit Inputs Below ######################      
            ...
            rcv_output.loc[len(rcv_output)] = [turnout_list[index], crossover_vec, cvap_vec, i] + list(scen_output.values())

rcv_output.to_csv("Lowell_rcv_output_3district.csv")
